...
public class EventWorker {
	...
	private final ProviderEventParser eventParser;
	...
	public EventWorker(ValidationPersistenceService validationPersistence, ProviderEventParser eventParser,
			ValidationDecisionService validationDecisionService,
			ProviderValidationResultService providerValidationResultService, ProviderValidationEventPersistenceService eventPersistenceService, List<ProviderEventParser> parsers) {
		...
		this.eventPersistenceService = eventPersistenceService;
		  ...
		  this.parserMap = parsers.stream()
	                .collect(Collectors.toMap(
	                        ProviderEventParser::getProvider,
	                        Function.identity()
	                ));
	}
	...
	private void processSingleEvent(ProviderValidationEventEntity event) {
		...
		if (request.getExecutionStatus().equals(ExecutionStatus.COMPLETED.toString())) {
			event.markSkipped();
			 eventPersistenceService.save(event);
			return;
		}
		
		ProviderEventParser parser =
                parserMap.get(event.getProvider());
		...
		if (parser == null) {
         ...
         if (event.getEventType() == ProviderEventType.WEBHOOK) {
             providerResult =
                     parser.parseWebhook(event.getRawPayload());
         } else {
             providerResult =
                     parser.parseApiResponse(event.getRawPayload());
         }
